...
const SurveyManagement: React.FC = () => {
  ...
  function handleViewAnalytics(surveyId: string) {
    navigate(`/admin/surveys/${surveyId}/analytics`);
  }
  ...
  return (
    <AdminLayout>
      <DashboardBg>
        <div className="survey-management" style={{ maxWidth: 900, margin: '0 auto', padding: '32px 0', background: '#fff8ee', borderRadius: 24, minHeight: '100vh' }}>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="surveys-list" style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              {filtered.map(s => (
                <div
                  key={s.id}
                ...
                >
                  ...
                  <div style={{ position: 'absolute', right: 18, top: 18, display: 'flex', gap: 10 }}>
                    <button 
                      ...
                      style={{ background: 'none', border: 'none', color: '#3498db', fontWeight: 700, cursor: 'pointer', fontSize: 15 }}
                    >
                      Analytics
                    </button>
                    <button 
                      onClick={() => handleEdit(s)} 
                      style={{ background: 'none', border: 'none', color: '#552a47', fontWeight: 700, cursor: 'pointer', fontSize: 15 }}
                    >
                      Edit
                    </button>
                    <button 
          ...
          {showModal && (
            <div className="modal-overlay" style={{
              ...
              <div className="modal-content" style={{ background: '#fff', borderRadius: 16, padding: '2rem 2.5rem', minWidth: 340, boxShadow: '0 4px 24px #e5d6c1', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', gap: 16, justifyContent: 'flex-end' }}>
                  <button 
                    onClick={() => { setShowModal(false); setEditSurvey(null); }} 
                    style={{ background: '#f7f2f5', border: 'none', color: '#552a47', fontWeight: 700, borderRadius: 8, padding: '10px 20px', fontSize: 15, cursor: 'pointer' }}
                  >
                    Cancel
                  </button>
                  <button 
                    onClick={handleSave} 
                    style={{ background: '#552a47', border: 'none', color: '#fff', fontWeight: 700, borderRadius: 8, padding: '10px 22px', fontSize: 15, cursor: 'pointer' }}
                  >
                    {editSurvey ? 'Update' : 'Add'}
                  </button>
                </div>
              </div>
            </div>
