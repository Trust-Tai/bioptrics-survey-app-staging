import React from 'react';
import styled from 'styled-components';
import { 
  LineChart, 
  Line, 
  XAxis, 
  YAxis, 
  CartesianGrid, 
  Tooltip, 
  ResponsiveContainer,
  ReferenceLine 
} from 'recharts';

/**
 * Interface for trend data used in the engagement trend line chart
 */
export interface TrendData {
  name: string;
  score: number;
}

/**
 * Props for the EngagementTrendLine component
 */
export interface EngagementTrendLineProps {
  data: TrendData[];
  isLoading: boolean;
  isBlurred: boolean;
}

const Container = styled.div<{ isBlurred: boolean }>`
  filter: ${props => props.isBlurred ? 'blur(4px)' : 'none'};
  transition: filter 0.3s ease;
`;

const Title = styled.h3`
  font-size: 16px;
  font-weight: 600;
  color: #1c1c1c;
  margin-bottom: 16px;
`;

const ChartContainer = styled.div`
  width: 100%;
  height: 300px;
`;

const LoadingOverlay = styled.div`
  display: flex;
  justify-content: center;
  align-items: center;
  height: 300px;
  background: rgba(255, 255, 255, 0.8);
`;

const LoadingSpinner = styled.div`
  border: 3px solid #f3f3f3;
  border-top: 3px solid #552a47;
  border-radius: 50%;
  width: 30px;
  height: 30px;
  animation: spin 1s linear infinite;
  
  @keyframes spin {
    0% { transform: rotate(0deg); }
    100% { transform: rotate(360deg); }
  }
`;

const AverageBadge = styled.div`
  display: inline-block;
  padding: 4px 8px;
  border-radius: 4px;
  background-color: #f8f9fa;
  color: #1c1c1c;
  font-size: 14px;
  font-weight: 600;
  margin-left: 12px;
`;

const CustomTooltip = styled.div`
  background-color: #fff;
  border: 1px solid #e2e8f0;
  border-radius: 4px;
  padding: 10px;
  box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1);
`;

const Card = styled.div`
  background: #ffffff;
  border-radius: 8px;
  box-shadow: 0 1px 6px rgba(0, 0, 0, 0.05);
  padding: 20px;
  margin-bottom: 24px;
`;

/**
 * EngagementTrendLine component for displaying engagement scores over time
 * Shows a line chart with a reference threshold line and average score
 */
const EngagementTrendLine: React.FC<EngagementTrendLineProps> = ({ data, isLoading, isBlurred }) => {
  // Calculate average score
  const averageScore = data.length 
    ? (data.reduce((sum, item) => sum + item.score, 0) / data.length).toFixed(1)
    : '0.0';
  
  return (
    <Card>
      <Container isBlurred={isBlurred}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <Title>Engagement Trend Line</Title>
          <AverageBadge>Average: {averageScore}/5</AverageBadge>
        </div>
        
        {isLoading ? (
          <LoadingOverlay>
            <LoadingSpinner />
          </LoadingOverlay>
        ) : (
          <ChartContainer>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={data}
                margin={{ top: 20, right: 30, left: 20, bottom: 5 }}
              >
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="name" />
                <YAxis domain={[0, 5]} ticks={[0, 1, 2, 3, 4, 5]} />
                <Tooltip
                  content={({ active, payload, label }) => {
                    if (active && payload && payload.length) {
                      return (
                        <CustomTooltip>
                          <p style={{ margin: '0 0 5px', fontWeight: 'bold' }}>{label}</p>
                          <p style={{ margin: '0', color: '#552a47' }}>
                            Score: {payload[0].value}/5
                          </p>
                        </CustomTooltip>
                      );
                    }
                    return null;
                  }}
                />
                <ReferenceLine y={3} stroke="#f39c12" strokeDasharray="3 3" label="Threshold" />
                <Line 
                  type="monotone" 
                  dataKey="score" 
                  stroke="#552a47" 
                  strokeWidth={2} 
                  dot={{ 
                    r: 6, 
                    stroke: '#552a47', 
                    strokeWidth: 2, 
                    fill: 'white' 
                  }} 
                  activeDot={{ 
                    r: 8, 
                    stroke: '#552a47', 
                    strokeWidth: 2, 
                    fill: '#552a47' 
                  }} 
                />
              </LineChart>
            </ResponsiveContainer>
          </ChartContainer>
        )}
      </Container>
    </Card>
  );
};

export default EngagementTrendLine;
