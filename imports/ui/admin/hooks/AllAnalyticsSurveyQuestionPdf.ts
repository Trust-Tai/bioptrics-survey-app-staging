...
const fetchMultipleSurveysAndGenerate = (
  ...
  const combinedSurveyData: {
    ...
    sectionQuestions: Array<{id: string, sectionId: string, text: string}>
  } = {
    surveySections: [],
    sectionQuestions: []
  };
  
  // Track how many surveys we've processed
  let surveysProcessed = 0;
  const totalSurveys = options.surveyIds?.length || 0;
  
  console.log(`Fetching data for ${totalSurveys} surveys`);
  
  // Process each survey ID
  options.surveyIds?.forEach(surveyId => {
    Meteor.call('surveys.getSurvey', surveyId, (error: any, surveyData: any) => {
      if (error) {
        ...
        console.log(`Fetched survey data for ${surveyId}:`, surveyData);
        
        // Create a section for this survey
        const surveySection: {id: string, name: string, description: string} = {
          id: `survey-${surveyId}`,
          name: surveyData.title || `Survey ${surveyId}`,
          description: surveyData.description || 'Survey questions and responses'
        };
        
        // Add the survey as a section
        combinedSurveyData.surveySections.push(surveySection);
        
        // Filter questions for this survey
        // This is a simplified approach - in a real implementation, you'd need to
        // ensure questions are properly associated with their surveys
        const surveyQuestions = analyticsQuestions.filter(q => {
          // Try to match questions to this survey
          // This logic may need to be adjusted based on your data structure
          return q.questionId.includes(surveyId) || 
                 (q as any).surveyId === surveyId;
        });
        
        console.log(`Found ${surveyQuestions.length} questions for survey ${surveyId}`);
        
        // Add questions to the section
        surveyQuestions.forEach(q => {
          const questionItem: {id: string, sectionId: string, text: string} = {
            id: q.questionId,
            sectionId: `survey-${surveyId}`,
            text: q.questionText
          };
          combinedSurveyData.sectionQuestions.push(questionItem);
        });
      }
      ...
      // When all surveys are processed, generate the PDF
...
export const exportQuestionsToPDF = (
  ...
  try {
    ...
    if (options?.surveyIds && options.surveyIds.length > 1) {
      console.log('Multiple surveys detected, using fetchMultipleSurveysAndGenerate');
      fetchMultipleSurveysAndGenerate(analyticsQuestions, options);
      return;
    }
    
    // If single surveyId is provided, fetch the complete survey data
    if (options?.surveyId) {
...
// Function to generate PDF with sections (enhanced implementation)
const generatePDFWithSections = (
  ...
  try {
    ...
    // Check if we're dealing with multiple surveys
    const isMultipleSurveys = options?.surveyIds && options.surveyIds.length > 1;
    ...
    surveySections.forEach((section: any) => {
      if (section.id && questionsBySection[section.id] && questionsBySection[section.id].length > 0) {
        ...
        // Format section name based on context
        let sectionName = '';
        if (isMultipleSurveys) {
          // For multiple surveys, use the survey name directly
          sectionName = section.name || `Survey ${section.id.replace('survey-', '')}`;
        } else {
          // For single survey, prefix with "Standard" if it has a name
          sectionName = section.name ? "Standard : " + section.name : 'Section';
        }
    ...
    if (questionsWithoutSection.length > 0) {
      ...
      reportData.parentTags.push({
        ...
        name: isMultipleSurveys ? 'Uncategorized Questions' : 'General Questions',
    ...
    if (reportData.parentTags.length === 0) {
      ...
      reportData.parentTags.push({
        id: 'survey-questions',
        name: isMultipleSurveys ? 'All Survey Questions' : 'Survey Questions',
