...
import { QuestionResponseStats } from '../../../features/surveys/api/questionResponseMethods';
...
const AllOverviewTab: React.FC<OverviewTabProps> = ({ analyticsData, isLoading, surveyId, filterParams }) => {
  ...
  // Fetch question response stats when surveyId or filterParams change
  useEffect(() => {
    setIsLoadingStats(true);
    
    // Use provided filterParams or create a default one based on surveyId
    const effectiveFilterParams = filterParams || (surveyId ? { surveyIds: [surveyId] } : {});
    
    Meteor.call('getFilteredQuestionResponseStats', effectiveFilterParams, (error: any, result: QuestionResponseStats) => {
      setIsLoadingStats(false);
      if (error) {
        console.error(`Error fetching filtered question response stats:`, error);
      } else {
        console.log(`Received filtered question response stats:`, result);
