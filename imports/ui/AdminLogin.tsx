...
export const AdminLogin: React.FC<AdminLoginProps> = ({ onAdminAuth }) => {
  ...
  useEffect(() => {
    ...
    if (adminToken && Meteor.userId()) {
      // User is already authenticated, show redirecting and redirect to Home page
      ...
      Meteor.call('admin.getToken', (err: any, res: any) => {
        if (err) {
          ...
          if (redirectTo) {
            ...
            if (currentHost.includes('pulse-lms.bioptrics.com')) {
...
const adminToken = localStorage.getItem('admin_jwt');
if (isRedirecting || adminToken && Meteor.userId()) {
  return (
    <AuthBg>
